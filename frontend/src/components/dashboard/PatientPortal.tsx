...
export default function PatientPortal({ patient, onClose }: PatientPortalProps) {
    ...
    useEffect(() => {
        async function fetchInteractions() {
            ...
            setError(null);
            try {
                const res = await fetch(API_ENDPOINTS.interactionsByPatient(patient.id));
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 z-50 flex items-center justify-center">
            <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl p-6 relative">
                ...
                <div>
                    ...
                    {loading ? <div>Loading...</div> : error ? <div className="text-red-600">{error}</div> : <PatientHistoryTimeline interactions={interactions} />}
                </div>
            </div>
        </div>
    );
