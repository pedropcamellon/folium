...
import { apiJson, apiRequest, API_ENDPOINTS } from "@/lib/api";
...
export function ChartReviewSection({ interactionId }: ChartReviewSectionProps) {
        ...
        async function loadReview() {
            ...
            try {
                const response = await apiRequest(
                    API_ENDPOINTS.interactionChartReview(interactionId)
                );
    ...
    return (
        <section className="border-t pt-4" aria-labelledby="chart-review-heading">
            <div className="flex items-center justify-between gap-3">
                <div>
                    <h3 id="chart-review-heading" className="text-sm font-semibold">
                        Chart Review Draft
                    </h3>
                    <p className="text-sm text-muted-foreground">
                        Draft support based on the selected interaction and available chart context.
                    </p>
                </div>
                <Button
                    type="button"
                ...
                >
                    {review ? <RefreshCw aria-hidden="true" /> : <FileSearch aria-hidden="true" />}
                    {review ? "Generate new draft" : "Generate draft"}
                </Button>
            </div>

            {error ? <p className="mt-3 text-sm text-destructive">{error}</p> : null}
            {isLoading ? <p className="mt-3 text-sm text-muted-foreground">Loading review...</p> : null}
            {review?.status === CHART_REVIEW_STATUS.FAILED ? (
                <p className="mt-3 text-sm text-destructive">
                    {review.failureMessage || "The draft review could not be completed."}
                </p>
            ) : null}
            {review?.status === CHART_REVIEW_STATUS.QUEUED ||
            review?.status === CHART_REVIEW_STATUS.RUNNING ? (
                <p className="mt-3 text-sm text-muted-foreground">Draft review is processing.</p>
            ) : null}
                ...
                <div className="mt-3 space-y-3 text-sm">
                    ...
                    {review.reasoning ? (
                        <div>
                            ...
                            <p className="mt-1 text-muted-foreground">{review.reasoning}</p>
                        </div>
                    ) : null}
                        ...
                        <div>
                            ...
                            <ul className="mt-1 space-y-1 text-xs text-muted-foreground">
                                {review.sourceRefs.map((sourceRef) => (
                                    <li key={`${sourceRef.sourceType}-${sourceRef.resourceId}-${sourceRef.contentRole}`}>
                    ...
                    {review.reviewFlags.length ? (
                        <ReviewList title="Review flags" items={review.reviewFlags} />
                    ) : null}
                </div>
            ) : null}
        </section>
...
function formatSourceReference(sourceRef: ChartReview["sourceRefs"][number]): string {
